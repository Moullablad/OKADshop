...
function DELETE($ID){
	...
	$common->delete('invoice_detail', "WHERE id_invoice=".$ID );
	$common->delete('invoice_carrier', "WHERE id_invoice=".$ID );
	$common->delete('invoices', "WHERE id=".$ID );
	echo '<script>window.location.href="?module=invoices"</script>';
}
//exit if delete action
if ( $_GET['action'] == "delete" ) return;

function View($ID){
	//echo '<script>window.location.href="../pdf/invoice.php?id_invoice='. $ID .'"</script>';
}
 ...
 *=============================================================
 * UPDATE MODE
 * This part well apply when you go to edit an invoice from list
 * EX: [WewSite]index.php?module=invoices&action=edit&id=[1]
 *=============================================================
 */
...
$invoices_list = '<script>window.location.href="?module=invoices"</script>';
if ( $id_invoice <= 0 ) echo $invoices_list;

//get id_customer
$id_customer = $common->select('invoices', array('id_customer'), "WHERE id=".$id_invoice );
if( !$id_customer[0] ) echo $invoices_list;
$id_customer = $id_customer[0]['id_customer'];

//ckeck if invoice exist
$o = $common->get_invoice($id_invoice, $id_customer);
if( !$o ) echo $invoices_list;

//get product list
$products = $common->select('products', array('id', 'name'));

//payment_methodes
$payment_methodes = $common->select('payment_methodes', array('id', 'value') );

//prepare vars
$currency_sign = $o['invoice']['currency_sign'];


//invoice total
$voucher_code  	 = $o['invoice']['voucher_code'];
$voucher_value 	 = $o['invoice']['voucher_value']; 
$global_discount = $o['invoice']['global_discount']; 
$avoir 					 = $o['invoice']['avoir'];
...
<div class="top-menu padding0">
  <div class="top-menu-title">
    <h3><i class="fa fa-credit-card"></i> <?=l("Edition de la Facture", "core");?>
    <span class="badge badge-default"><?php echo $o['invoice']['reference']; ?></span>
		<span class="badge"><?=l("N°", "core");?><?php echo sprintf("%06d", $o['invoice']['id']);?></span>
	</h3>
  </div>
  <a href="?module=invoices" class="btn btn-default pull-right"><?=l("Términer", "core");?></a>
</div><br>
<input type="hidden" value="<?=$id_invoice;?>" id="id_invoice">
...
<div class="col-sm-12 padding0" style="margin-bottom:10px;border-bottom:1px solid #A5245E;">
	<ul class="nav nav-tabs bg-white">
		...
		<li class="pull-right">
			<a target="_blank" href="../pdf/invoice.php?id_invoice=<?=$o['invoice']['id'];?>&id_customer=<?=$o['invoice']['id_customer'];?>" class="btn"><i class="fa fa-file-pdf-o"></i> <?=l("Apérçu la Facture", "core");?></a>
		</li>
	</ul>
</div>
	...
	<div class="tab-pane active" id="step-1">
		<div class="panel panel-default">
			<div class="panel-body">
				...
				<form class="form-horizontal" id="product_form" method="post" action="ajax/invoices/form/product.php">
					<table id="add_product" class="table table-striped table-bordered" cellspacing="0" width="100%">
					<input type="hidden" value="<?=$id_invoice;?>" name="id_invoice" class="id_invoice">
					...
					<tbody>
			          ...
			          <select name="id_declinaisons" class="form-control hidden" id="based_attributs" disabled required>
		    ...
		    <div class="panel-subheading">
					...
					<strong><?=l("Produits dans cette Facture", "core");?></strong>
				</div>

				<table id="invoice_products" class="table table-striped table-bordered" cellspacing="0" width="100%">
				<thead>
					<tr>
						<th><?=l("Nom de produit", "core");?></th>
						<th width="190"><?=l("Référence", "core");?></th>
				...
				<div class="col-sm-3 col-xs-12 pull-right padding0">
						<table class="table table-striped table-bordered" id="invoice_total">
							<tbody>
								<tr>
				...
				<form class="form-horizontal" method="post" action="">
					...
					<tbody>
			            ...
			            <?php endif; ?>
							...
							<td>
								<input type="text" name="payment[transaction_id]" value="" class="form-control" id="transaction_id"  placeholder="<?=l("ID de la transaction", "core");?>" required>
							</td>
							<td>
								<div class="input-group">
	...
	<div class="tab-pane" id="step-4">
		<div class="panel panel-default">
			<div class="panel-body">
				...
				<form class="form-horizontal" id="carrier_form" method="post" action="ajax/invoices/form/carrier.php">
					<input type="hidden" value="<?=$id_invoice;?>" id="id_invoice" name="id_invoice">
					<div class="form-group">
						<label class="col-sm-3 control-label"><?=l("Formule", "core");?></label>  
						<div class="col-sm-3">
							<input type="text" value="<?=(isset($o['invoice']['carrier_type'])) ? $o['invoice']['carrier_type'] : '';?>" class="form-control" id="formule" disabled>
						</div>
					</div>
					<div class="form-group">
...
$(document).ready(function(){
  ...
  $("form#product_form").submit(function(event){
    ...
    submit_ajax_form("product_form", function(data) {
    	if( data.row ) $( data.row ).prependTo("#invoice_products > tbody");
    	$('#product_form input').not('.id_invoice').val('');
    	...
    	os_message_notif('<?=l("Le produit a été enregistré avec succès", "core");?>');
    });
    return false;
  });
  ...
  $("form#carrier_form").submit(function(event){
	...
	//update product
	$('#invoice_products').on('click', '.update', function() {
		...
		$.ajax({
			...
			data: {product:product,id_invoice:id_invoice,id_customer:id_customer},
	...
	$('#based_product').on('change', function(){
		...
		$('#add_product input:not(".id_invoice")').val('');
	...
	//delete-product
	$('#invoice_products').on('click', '.delete', function(){
    ...
    $.ajax({
      type: "POST",
      data: {id:id,id_invoice:id_invoice,id_customer:id_customer},
      ...
      success: function(data){
      	...
      	os_message_notif( "<?=l('Le produit a été supprimer avec success.', 'admin');?>" );
				//remove row
				$('#invoice_products tbody tr#'+id).fadeOut(500, function() { 
      		$(this).remove(); 
      	});
      },
      error: function(jqXHR, textStatus, errorThrown){
...
function refresh_products_total(){
	...
	$.ajax({
    type: "POST",
    data: {id_invoice:id_invoice,id_customer:id_customer},
    url: ajaxurl + 'refresh/total.php',
    success: function(data){
    	var data = $.parseJSON(data);
    	$('table#invoice_total > tbody').empty().append( data );
    },
    error: function(jqXHR, textStatus, errorThrown){
    	get_error_message(textStatus, errorThrown);
    }
