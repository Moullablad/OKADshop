...
function ADD(){
	$user=new Form();
	$form = array(
		array('HIDDEN','module','50','module','module','tags'),
		array('HIDDEN','redirect','50','redirect','redirect','tags'),
    			...
    			array('DIVSTART','','left-block'),
    				...
    				array('DIVSTART','','block'),
    					array('DIVSTART','','form-group'),
    						array('LABEL', l('Nom de Tag', "core")/*Text*/,'control-label'/*class*/,''/*ID*/),
    						array('TEXT','name','form-control','form-control','name','','',''),
    					array('DIVEND'),


    					array('DIVSTART','','form-group'),
    						array('LABEL', l('language', "core")/*Text*/,'control-label'/*class*/,''/*ID*/),
    						array('COMBO','id_lang','form-control','id_lang', _DB_PREFIX_.'langs','id','name'),
    					array('DIVEND'),
    				array('DIVEND'),

    				array('DIVSTART','','block clearfix'),
			      	array('TITLE','','','','<h2>' . l('Vérifier tous vos information et cliquer sur le bouton save.', "core") .'</h2>'),
			        array('DIVSTART','','top-menu-button'),
			        	array('BUTTON',l("Enrigistrer la Tag", "core"),'send','btn btn-primary','send'),
...
function EDIT($ID){
	...
	$user=new Form();
	$form = array(
		array( _DB_PREFIX_.'tags'/*table*/,'id'/*id table*/,$ID),
	  array('HIDDEN','module','module',50,'module','module','tags'),
...
{
    $Post = array();
    array_push($Post,array( _DB_PREFIX_.'tags'/*table*/));
