...
class OS_Common {
   ...
   * @return html.
   **/
  public function get_module_page_content($pages, $module_name=""){
    if( !isset($_GET['slug']) ) return false;
    if( $_GET['slug'] != $module_name || !isset($_GET['page']) && !in_array($_GET['page'], $pages) ) return false;
   ...
   * @throws Exception
   */
  public function get_exception_error($e){
    try {
  ...
  //os_prefix_query
  function os_prefix_query($query)
  {
   ...
   * @throws Exception
   */
  public function select($table, $columns = array('*'), $condition = "")
  {
    try {
      global $DB;
  ...
  function os_getLoyalty($id_customer,$id_quotation){
    try{
  ...
  function updateColumn($table,$columns,$condition=""){
   try {
      $DB = Database::getInstance();
   ...
   * @return html
   */
  public function getCategories($parent,$cat_list)
  {
    $DB = Database::getInstance();
   ...
   * @throws Exception
   */
  public function uploadFiles($files,$uploadDir,$extensions=array('jpg', 'jpeg', 'png', 'gif'),$maxSize = 10)
  {
   ...
   * @throws Exception
   */
  public function uploadImage($image, $uploadDir, $title=null)
  {
    //Create directory if not exist
    if (!file_exists($uploadDir)) {
      mkdir($uploadDir, 0777, true);
   ...
   * @throws Exception
   */
  public function uploadDocument($file, $file_name=array('auto', 10), $uploadDir, $extensions="")
  {
    //Create directory if not exist
    if (!file_exists($uploadDir)) {
      mkdir($uploadDir, 0777, true);
   ...
   * @throws Exception
   */
  public function cropImage($uploadDir,$filename,$sizes=null,$quality=100)
  {
    //Vars
   ...
   * @throws Exception
   */
  function searching_multidimensional_arrays($array, $arr_key, $arr_val) {
   ...
   * @throws Exception
   */
  function array_searching($array, $field, $value)
  {
    if( !empty($array) )
    {
  ...
  public function save_mete_value($name, $value) {
    $DB = Database::getInstance();
    try {
  ...
  public function select_mete_value($name){
    $DB = Database::getInstance();
    try {
  ...
  public function select_meta_value($name){
    $DB = Database::getInstance();
    try {
      $query = "SELECT value FROM "._DB_PREFIX_."meta_value WHERE name = '$name'";
      if($rows = $DB->pdo->query($query)){
   ...
   * @throws Exception
   */
  public function get_quotation($id_quotation, $id_customer){
    try {
   ...
   * @throws Exception
   */
  public function get_order($id_order, $id_customer){
    try {
   ...
   * @throws Exception
   */
  public function get_invoice($id_invoice, $id_customer){
    try {
  ...
  //customer infos
  public function os_get_customer_infos($id_customer){
    global $DB;
   ...
   * @throws Exception
   */
  /*public function os_get_quotation_total($total, $buyprice, $weight){
    try {
   ...
   * @throws Exception
   */
  public function discount_percent($price, $discount){
    try {
   ...
   * @throws Exception
   */
  public function discount_devise($price, $discount){
    try {
      if( $discount <= 0 || $price <= 0 ){
        return 0;
      }else{
   ...
   * @throws Exception
   */
  public function tax_percent($price, $tax){
    try {
      if( $tax <= 0 || $price <= 0 ){
        return 0;
      }else{
   ...
   * @throws Exception
   */
  public function tax_devise($price, $tax){
    try {
      if( $tax <= 0 || $price <= 0 ){
        return 0;
      }else{
   ...
   * @throws Exception
   */
  public function get_file_name( $file_name, $file_dem="80x80" )
  {
    try {
   ...
   * @throws Exception
   */
  public function check_cart_rule( $code, $id_customer, $id_carrier, array $product_ids, $amount )
  {
    try {
   ...
   */
  function copyr($source, $dest, $excludes=array())
  {
    // Check for symlinks
    if (is_link($source)) {
  ...
  //get_dir_contents
  function get_dir_contents($dir, $exclude_dir=array(), &$results = array()){
    $files = scandir($dir);
  ...
  function os_resize_images_in_folder($file_path, $sizes=array())
  {
    try {
